...
const AddUsers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!institutionalIdRegex.test(form.institutionalId)) {
      setError("Institutional ID must be in the format R123456.");
      return;
    }
    try {
      setLoading(true);
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
        ...
        <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-[var(--color-surface)] rounded-[var(--radius-lg)] shadow-[var(--shadow-lg)] border border-[var(--color-border)]">
          ...
          {/* Form */}
          <form onSubmit={handleSubmit} className="px-6 sm:px-7 py-6 space-y-5">
            {error && (
            ...
            <div>
              ...
              <p className="mt-1.5 text-[11px] text-[var(--color-text-muted)]">
                Min 8 characters, include a number and a symbol. User can change
